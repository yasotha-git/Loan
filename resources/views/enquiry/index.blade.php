...
<body>
        ...
        <div class="container" style="flex-grow: 1; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            <div class="register-form-wrapper text-center">
                ...
                <div class="container" style="max-width: 800px;">
                    ...
                    <!-- Simple Pagination Links with Previous and Next buttons -->
    ...
    <script>
        function showUserModal(enquiryId) {
            // Make an AJAX request to get users with role_id == 2

            $.ajax({
                url: '/get-adminenquiry-details', // Your route to fetch enquiry details
                type: 'GET',
                data: {
                    enquiry_id: enquiryId
                },
                success: function(enquiryResponse) {
                    console.log("gooo", enquiryResponse);

                    const status = enquiryResponse.status;
                    const username = enquiryResponse.username;
                    $.ajax({
                        url: '/get-user-info', // This is the route to fetch users
                        type: 'GET',
                        success: function(response) {
                            // Dynamically populate the user dropdown
            ...
            `;
                            ...
                            // Make an AJAX request to get the current status and user_id for the enquiry


                            // Show SweetAlert with dropdowns and pre-populate values
                            Swal.fire({
                            ...
                            `,
                                ...
                                didOpen: () => {
                                    // Pre-select the current user and status
                                    $('#user').val(userId); // Pre-select the user
                                    $('#status').val(status); // Pre-select the status
                                },
                                preConfirm: () => {
                                    ...
                                    if (!userId || !status) {
                                        Swal.showValidationMessage('Please select both user and status');
                                        return false;
                                    }

                                    // Save the data (AJAX or form submission)
                                    $.ajax({
                                        url: '/update-enquiry', // Your route to update enquiry
                                        type: 'POST',
                                        data: {
